...
@pytest.mark.parametrize(
    ...
    [
        (ia_configure, "configure", ("conf",)),
        (ia_metadata, "metadata", ("met",)),
        (ia_upload, "upload", ("put", "send")),
        (ia_download, "download", ("dl", "get", "fetch")),
        (ia_delete, "delete", ("del",)),
        (ia_tasks, "tasks", ("tk",)),
        (ia_list, "list", ("l",)),
    ],
)
def test_fork_command_aliases(module, command, aliases):
    """The fork's additional command aliases survive the argparse migration."""
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers(dest="command")
    module.setup(subparsers)

    for alias in aliases:
        assert subparsers.choices[alias] is subparsers.choices[command]
